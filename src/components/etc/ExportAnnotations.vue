...
export default {
  ...
  methods: {
    ...
    async generateJSONExport(annotator) {
          ...
          annotation.text, // Text directly in the array
          {
            entities: annotation.entities.length
              ? annotation.entities.map((entity) => {
                  ...
                  const history = entity.history // Ensure history is initialized
                  ...
                  if (
                    entity.reviewed &&
                    history[history.length - 1][3] != annotator &&
                    history[history.length - 1][1] == entity.currentState &&
                    history[history.length - 1][0] == entity.labelClass.name
                  ) {
                    history.push([
                    ...
                    (entity.currentState == 'Candidate' || entity.currentState == 'Suggested') &&
                    history.length == 0
                  ) {
      ...
      try {
        // file saving logic
        this.$store.lastSavedTimestamp = new Date().toLocaleTimeString([], {
          hour: '2-digit',
          minute: '2-digit',
        })
        console.log(typeof window.rpc)
